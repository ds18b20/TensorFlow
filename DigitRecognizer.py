...
for step in range(max_iteration):
    ...
    sess.run(train_step, feed_dict={xs: xs_batch, ys: ys_batch, keep_prob: 0.95})
    if step % epoch == 0:
...
pre_list = []
test_path = r"MNIST_data/test.csv"
test_image = pd.read_csv(test_path).astype('uint8')  # 0-255
# test_image = test_image.values.reshape(-1, 1, 28, 28)  # no label column
test_image = (test_image / 255.0).astype('float32')
test_image = test_image.values
print('test_image.shape:', test_image.shape)
for image in test_image:
    pre_result = sess.run(prediction, feed_dict={xs: image.reshape([1, -1]), keep_prob: 1})
    pre_list.append(np.argmax(pre_result, axis=1))

label_col = np.array(range(1, len(pre_list) + 1))
output_col = np.squeeze(np.array(pre_list), axis=1)

generate_submission_csv(label_col, output_col, 'submission.csv')
print('OK')
